...
function traverseUpFileTreeAndPerformActionUntil(dirPath, endPath, action) {
...
function collectWorkspaceLibrarySecondaryEntryPoints(library, libraryPath, tsconfigPathAliases) {
    let libraryRootDirPath = libraryPath;
    traverseUpFileTreeAndPerformActionUntil((0, path_1.dirname)(libraryRootDirPath), devkit_1.workspaceRoot, (currentDirPath) => {
        if ((0, fs_1.existsSync)((0, path_1.join)(currentDirPath, 'package.json'))) {
    ...
    for (const alias of aliasesUnderLibrary) {
        ...
        traverseUpFileTreeAndPerformActionUntil(searchDir, libraryRootDirPath, (currentDirPath) => {
            if ((0, fs_1.existsSync)((0, path_1.join)(currentDirPath, 'ng-package.json'))) {
...
function shareWorkspaceLibraries(libraries, tsConfigPath) {
    ...
    for (const [key, paths] of Object.entries(tsconfigPathAliases)) {
        if (libraries && libraries.includes(key)) {
            ...
            collectWorkspaceLibrarySecondaryEntryPoints(key, pathToLib, tsconfigPathAliases).forEach(({ name, path }) => pathMappings.push({
                name,
                path,
            }));
            pathMappings.push({
